...
const ensureTelemetrySession = () => {
  ...
  telemetrySessionPromise = (async () => {
    ...
    if (!profile.year || !profile.tracks.length) throw new Error("profile_not_ready");
    ...
    const response = await fetch(`${TELEMETRY_API_URL}/v1/session`, {
      ...
      body: JSON.stringify({
        ...
        turnstileToken,
        year: profile.year,
        majors: profile.tracks,
        os: device.os,
        deviceType: device.deviceType,
        payload: telemetryPayload("session_start"),
      }),
      cache: "no-store",
    });
...
const telemetryPayload = (reason) => {
  ...
  return {
    version: 4,
    eventId: reason === "session_start"
      ? `${sessionId()}:session_start`
      : `${sessionId()}:${reason}:${sequence}`,
    reason,
    session: sessionId(),
...
window.addEventListener("ags-profile-saved", () => {
