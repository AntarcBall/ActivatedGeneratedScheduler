...
import { ProfessorTimetablePage } from './components/ProfessorTimetablePage';
import { RoomTimetablePage } from './components/RoomTimetablePage';
import LandingPage from './components/LandingPage';
import { translations } from './translations';

const normalizePath = (value: string) => value.replace(/\/+/g, '/').replace(/^\/+|\/+$/g, '');

const resolveSpecialRoute = () => {
    const base = normalizePath(import.meta.env.BASE_URL || '/');
    const path = normalizePath(window.location.pathname);
    const pathFromBase = base
    ...
    const rawRoute = pathFromBase || hashPath;
    let route = normalizePath(rawRoute);

    try {
        route = normalizePath(decodeURIComponent(rawRoute));
    } catch {
        route = normalizePath(rawRoute);
    }

    if (route.toLowerCase().startsWith('professor/')) {
        const rawProfessor = route.slice('professor/'.length);
        if (!rawProfessor) return null;

        try {
            const decoded = decodeURIComponent(rawProfessor);
            return {
                kind: 'professor' as const,
                value: decoded === 'kim-sohee' ? '김소희' : decoded,
            };
        } catch {
            return {
                kind: 'professor' as const,
    ...
    if (route.toLowerCase().startsWith('room/')) {
        const rawRoom = route.slice('room/'.length);
        if (!rawRoom) return null;

        try {
            return {
                kind: 'room' as const,
                value: decodeURIComponent(rawRoom),
            };
        } catch {
            return {
                kind: 'room' as const,
...
function App() {
    const specialRoute = resolveSpecialRoute();

    return (
        <AppProvider>
            {specialRoute?.kind === 'professor' ? (
                <ProfessorTimetablePage professorName={specialRoute.value} />
            ) : specialRoute?.kind === 'room' ? (
                <RoomTimetablePage roomName={specialRoute.value} />
            ) : (
                <Content />
            )}
        </AppProvider>
